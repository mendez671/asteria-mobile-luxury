...
export async function create_ticket(params: CreateTicketParams): Promise<{
  ...
  const missingRequirements = service.bookingRequirements.filter(
    req => !(req in params.requirements)
  );
  
  if (missingRequirements.length > 0) {
    throw new Error(`Missing required information: ${missingRequirements.join(', ')}`);
  }

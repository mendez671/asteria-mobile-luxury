...
export async function GET() {
  ...
  try {
    // Check 1: Secret Manager Integration
    let openaiApiKey: string;
    try {
      openaiApiKey = await getOpenAIKey();
      diagnostics.checks.secretManagerAccess = 'SUCCESS';
      diagnostics.checks.apiKeyPresent = !!openaiApiKey;
      diagnostics.checks.apiKeyLength = openaiApiKey?.length || 0;
      diagnostics.checks.apiKeyFormat = openaiApiKey?.startsWith('sk-') || false;
    } catch (error) {
      diagnostics.checks.secretManagerAccess = `FAILED: ${error}`;
      diagnostics.checks.apiKeyPresent = false;
